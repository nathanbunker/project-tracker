...
import com.myapp.api.v1.resource.dto.ActionDto;
import com.myapp.api.v1.resource.dto.ApiErrorResponse;
import com.myapp.api.v1.resource.dto.CreateProposalRequest;
import com.myapp.api.v1.resource.dto.ProjectDto;
import com.myapp.api.v1.resource.dto.ProposalDto;
...
public class ProjectsResource extends BaseApiResource {
    ...
    public List<ActionDto> listActions(@PathParam("projectId") int projectId) {
        requirePositiveId(projectId, "projectId");
        ApiRequestContext.ApiClientInfo client = requireClient();
        String providerId = requireProviderId(client);
        requireProject(providerId, projectId);
        List<ProjectAction> actions = proposalService.listActionsForProject(providerId, projectId);
        List<ActionDto> result = new ArrayList<ActionDto>();
        for (ProjectAction action : actions) {
            result.add(ActionDto.from(action));
        }
        return result;
    }
