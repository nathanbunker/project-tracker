...
public class BudgetServlet extends ClientServlet
{
  ...
  protected void processRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
  {
    ...
    try
    {
      ...
      Query query = dataSession.createQuery("from BudgetAccount where providerId = ? order by accountLabel");
      ...
      {
        out.println("  <tr class=\"boxed\">");
        out.println("    <td class=\"boxed\"><a href=\"BudgetServlet?accountId=" + budgetAccount.getAccountId() + "\" class=\"button\">"
            + budgetAccount.getAccountLabel() + "</a></td>");
        out.println("    <td class=\"boxed\">" + sdf.format(budgetAccount.getBalanceDate()) + "</td>");
        out.println("    <td class=\"boxed\"><span class=\"right\">" + MoneyUtil.format(budgetAccount.getBalanceAmount()) + "</span></td>");
        out.println("  </tr>");
      }
      out.println("  <tr class=\"boxed\">");
      out.println("    <td class=\"boxed\" colspan=\"3\"><span class=\"right\"><font size=\"-1\"><a href=\"BudgetAccountEditServlet\" class=\"box\">Add New Account</a></font></span></td>");
      out.println("  </tr>");
      ...
      if (request.getParameter("accountId") != null)
      {
        ...
        query = dataSession
            .createQuery("from BudgetTrans where budgetMonth = ? and budgetItem.budgetAccount = ? order by transDate, transAmount desc");
        ...
        if (nextMonth.after(today))
        {
          ...
          if (prevBudgetMonth != null || prevYearBudgetMonth != null)
          {
            ...
            // added to it
            query = dataSession.createQuery("from BudgetItem where budgetAccount = ? and itemStatus = 'M' or itemStatus = 'Y'");
            query.setParameter(0, budgetAccount);
            ...
            for (BudgetItem potentialBudgetItem : potentialBudgetItems)
            {
              ...
              if (!found)
              {
                boolean isMonthly = potentialBudgetItem.getItemStatus().equals(BudgetItem.ITEM_STATUS_MONTHLY);
                if ((prevBudgetMonth != null && isMonthly)
                    || (prevYearBudgetMonth != null && MonthUtil.sameMonthOnly(potentialBudgetItem.getLastDate(), monthDate)))
                {
                  ...
                  if (prevBudgetTrans != null)
                  {
                    ...
                    budgetTrans.setTransDate(MonthUtil.thisMonth(monthDate, prevBudgetTrans.getTransDate()));
                  ...
                  } else
                  {
                    ...
                    budgetTrans.setTransDate(MonthUtil.thisMonth(monthDate, potentialBudgetItem.getLastDate()));
            ...
            {
              query = dataSession
                  .createQuery("from BudgetTrans where budgetMonth = ? and budgetItem.budgetAccount = ? order by transDate, transAmount asc");
              query.setParameter(0, budgetMonth);
              query.setParameter(1, budgetAccount);
        ...
        {
          String link = "BudgetServlet?accountId=" + budgetAccount.getAccountId() + "&monthDate=" + sdf.format(prevDate) + "";
        ...
        {
          String link = "BudgetServlet?accountId=" + budgetAccount.getAccountId() + "&monthDate=" + sdf.format(MonthUtil.getNextMonth(monthDate))
              + "";
        ...
        Date startDate = budgetAccount.getStartDate().after(monthDate) ? budgetAccount.getStartDate() : monthDate;
        out.println("  <tr class=\"boxed\">");
        out.println("    <td class=\"boxed\" colspan=\"5\">Starting Balance</td>");
        out.println("    <td class=\"boxed\"><span class=\"right\">" + MoneyUtil.format(endingBalance) + "</span></td>");
        out.println("  </tr>");
        for (BudgetTrans budgetTrans : budgetTransList)
        {
          endingBalance += budgetTrans.getTransAmount();
          String priorityCode = budgetTrans.getBudgetItem().getPriorityCode();
          totalsByPriorityMap.put(priorityCode, totalsByPriorityMap.get(priorityCode) + budgetTrans.getTransAmount());
          out.println("  <tr class=\"boxed\">");
          out.println("    <td class=\"boxed\">" + shortSdf.format(budgetTrans.getTransDate()) + "</td>");
          out.println("    <td class=\"boxed\"><a href=\"BudgetItemEditServlet?accountId=" + budgetAccount.getAccountId() + "&monthDate="
              + sdf.format(monthDate) + "&transId=" + budgetTrans.getTransId() + "\" class=\"button\">" + budgetTrans.getBudgetItem().getItemLabel()
              + "</a></td>");
        ...
        trans.commit();
        out.println("  <tr class=\"boxed\">");
        out.println("    <td class=\"boxed\" colspan=\"7\"><span class=\"right\"><font size=\"-1\"><a href=\"BudgetItemEditServlet?accountId="
            + budgetAccount.getAccountId() + "&monthDate=" + sdf.format(monthDate) + "\" class=\"box\">New Budget Item</a></font></span></td>");
        out.println("  </tr>");
        out.println("</table>");
        ...
        Collections.sort(budgetTransList, new Comparator<BudgetTrans>() {
          ...
          {
            if (arg0.getBudgetItem().getPriorityCode().equals(arg1.getBudgetItem().getPriorityCode()))
            {
            ...
            {
              return arg0.getBudgetItem().getPriorityCode().compareTo(arg1.getBudgetItem().getPriorityCode());
            }
          };
        });
        int priorityTotal = 0;
        for (BudgetTrans budgetTrans : budgetTransList)
        {
          ...
          {
            out.println("  <tr class=\"boxed\">");
            out.println("    <th class=\"title\" colspan=\"6\">" + BudgetItem.getPriorityCodeLabel(budgetTrans.getBudgetItem().getPriorityCode())
                + "</th>");
            out.println("  </tr>");
            out.println("  <tr class=\"boxed\">");
            out.println("    <th class=\"boxed\">Item</th>");
          ...
          out.println("    <td class=\"boxed\"><span class=\"right\">" + MoneyUtil.format(priorityTotal) + "</span></td>");
        ...
        out.println("    <td class=\"boxed\">" + MoneyUtil.format(totalsByPriorityMap.get(BudgetItem.PRIORITY_CODE_INCOME)) + "</td>");
        out.println("  </tr>");
        out.println("  <tr>");
        ...
        out.println("    <td class=\"boxed\">"
            ...
            + MoneyUtil.format(totalsByPriorityMap.get(BudgetItem.PRIORITY_CODE_DONATIONS)
                + totalsByPriorityMap.get(BudgetItem.PRIORITY_CODE_COMMITTED) + totalsByPriorityMap.get(BudgetItem.PRIORITY_CODE_SCHEDULED)
                + totalsByPriorityMap.get(BudgetItem.PRIORITY_CODE_FLEXIBLE) + totalsByPriorityMap.get(BudgetItem.PRIORITY_CODE_DEBT_PAYOFF)
                + totalsByPriorityMap.get(BudgetItem.PRIORITY_CODE_SAVINGS)) + "</td>");
        out.println("  </tr>");
        out.println("  <tr class=\"boxed\">");
        ...
        out.println("    <td class=\"boxed\">" + MoneyUtil.format(totalsByPriorityMap.get(BudgetItem.PRIORITY_CODE_COMMITTED)) + "</td>");
        out.println("  </tr>");
        out.println("  <tr>");
        out.println("    <th class=\"boxed\">Flexible</th>");
        out.println("    <td class=\"boxed\">" + MoneyUtil.format(totalsByPriorityMap.get(BudgetItem.PRIORITY_CODE_FLEXIBLE)) + "</td>");
        out.println("  <tr>");
        out.println("  <tr>");
        out.println("    <th class=\"boxed\">Scheduled</th>");
        out.println("    <td class=\"boxed\">" + MoneyUtil.format(totalsByPriorityMap.get(BudgetItem.PRIORITY_CODE_SCHEDULED)) + "</td>");
        out.println("  </tr>");
        out.println("  </tr>");
        out.println("    <th class=\"boxed\">Donations</th>");
        out.println("    <td class=\"boxed\">" + MoneyUtil.format(totalsByPriorityMap.get(BudgetItem.PRIORITY_CODE_DONATIONS)) + "</td>");
        out.println("  </tr>");
        out.println("  <tr>");
        out.println("    <th class=\"boxed\">Debt Payoff</th>");
        out.println("    <td class=\"boxed\">" + MoneyUtil.format(totalsByPriorityMap.get(BudgetItem.PRIORITY_CODE_DEBT_PAYOFF)) + "</td>");
        out.println("  </tr>");
        out.println("  <tr>");
        out.println("    <th class=\"boxed\">Savings</th>");
        out.println("    <td class=\"boxed\">" + MoneyUtil.format(totalsByPriorityMap.get(BudgetItem.PRIORITY_CODE_SAVINGS)) + "</td>");
        out.println("  </tr>");
        out.println("</table>");
        out.println("<h2>Balance Account</h2>");
        out.println("<p><a href=\"BudgetBalanceServlet?accountId=" + budgetAccount.getAccountId() + "\" class=\"box\">Balance Account</a></p>");
        out.println("<h2>Manage Transaction Records</h2>");
        out.println("<p><a href=\"BudgetTransRecordServlet?accountId=" + budgetAccount.getAccountId() + "\" class=\"box\">Manage Transaction Records</a></p>");
      }
      out.println("</div>");
      printHtmlFoot(out);
  ...
  protected static BudgetMonth getOrCreateBudgetMonth(Session dataSession, BudgetAccount budgetAccount, Date monthDate)
  {
    Query query;
  ...
  protected static BudgetMonth getBudgetMonth(Session dataSession, BudgetAccount budgetAccount, Date monthDate)
  {
    Query query;
    BudgetMonth budgetMonth = null;
    query = dataSession.createQuery("from BudgetMonth where budgetAccount = ? and monthDate = ?");
