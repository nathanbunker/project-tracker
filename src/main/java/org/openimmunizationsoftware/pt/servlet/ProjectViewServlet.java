  ...
  protected void processRequest(HttpServletRequest request, HttpServletResponse response)
    ...
    try {
      ...
      ProjectProvider projectProvider =
          (ProjectProvider) dataSession.get(ProjectProvider.class, project.getProviderId());
      ...
      for (ProjectContactAssigned projectContactAssigned : projectContactAssignedList) {
        ...
        if (projectContact.getEmail() != null && !projectContact.getEmail().equals("")) {
          ...
          out.println(" <a href=\"mailto:" + projectContact.getEmail() + "?subject="
              + encode(projectName + " Project") + "\">" + projectContact.getEmail() + "</a>");
        }
        out.println("</td>");
        out.println("  </tr>");
      }
      ...
      if (projectActionList.size() > 0) {
        ...
        for (ProjectAction projectAction1 : projectActionList) {
          ...
          out.println("    <td class=\"inside\">"
              + projectAction1.getNextDescriptionForDisplay(null) + "</td>");
          out.println("  </tr>");
        }
        out.println("</table>");
      } else {
