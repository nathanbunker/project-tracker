...
public class HomeServlet extends ClientServlet
{
      ...
      if (webUser != null) {
        ...
        if (action != null) {
          if (action.equals("DoToday") || action.equals("DoNextWeek") || action.equals("DoTomorrow")) {
            ...
            try {
              ...
              if (oldDateDue != null && oldDateDue.before(projectAction.getNextDue())) {
                if (projectAction.getNextActionType() != null) {
                  if (projectAction.getNextActionType().equals(ProjectNextActionType.COMMITTED_TO)) {
                    projectAction.setNextActionType(ProjectNextActionType.OVERDUE_TO);
                  } else if (projectAction.getNextActionType().equals(ProjectNextActionType.MIGHT)) {
              ...
              if (request.getParameter("nextTimeEstimate") != null
                ...
                try {
                  projectAction.setNextTimeEstimate(Integer.parseInt(request.getParameter("nextTimeEstimate")));
            ...
            if (switched) {
              ProjectProvider projectProvider = (ProjectProvider) dataSession.get(ProjectProvider.class,
                  webUser.getProviderId());
        ...
        if (childWebUserList != null) {
          ...
          if (webUser.getParentWebUser() != null) {
            ProjectProvider projectProvider = (ProjectProvider) dataSession.get(ProjectProvider.class, webUser
                .getParentWebUser().getProviderId());
            out.println("  <tr class=\"boxed\">");
            ...
            out.println("    <td class=\"boxed\"><a href=\"" + switchLink + "\" class=\"button\">" + switchLabel
                + "</a></td>");
            out.println("  </tr>");
          } else {
            ProjectProvider projectProvider = (ProjectProvider) dataSession.get(ProjectProvider.class,
  ...
  protected static void printActionsDue(WebUser webUser, PrintWriter out, Session dataSession, String nextActionType,
      Date nextDue, boolean showLink, boolean showMenu) {
    SimpleDateFormat sdf1 = new SimpleDateFormat("MM/dd/yyyy");
    Query query = dataSession
        .createQuery("from ProjectAction where providerId = ? and (contactId = ? or nextContactId = ?) and nextActionId = 0 and nextDescription <> '' order by nextDue, priority_level DESC, nextTimeEstimate, actionDate");
    ...
    if (projectActionListOverdue.size() > 0) {
      ...
      for (ProjectAction projectAction : projectActionListOverdue) {
        printActionOverdueLine(webUser, out, sdf1, nextActionType, nextDue, projectAction, showLink);
      }
    ...
    printDueTable(webUser, out, sdf1, nextActionType, nextDue, projectActionList, cIndicated, cToday, cTomorrow,
        showLink);
    printDueTable(webUser, out, sdf1, nextActionType, nextDueTomorrow, projectActionList, nextDueTomorrowCalendar,
        cToday, cTomorrow, showLink);
  }

  private static void printScript(PrintWriter out, SimpleDateFormat sdf1, String nextActionType, Date nextDue) {
    out.println("        <script>");
  ...
  private static void printMenuForm(PrintWriter out, SimpleDateFormat sdf1, String nextActionType, Date nextDue) {
    ...
    out.println("            I: <font size=\"-1\"><a href=\"HomeServlet?nextActionType=" + ProjectNextActionType.WILL
    ...
    out.println("            <a href=\"HomeServlet?nextActionType=" + ProjectNextActionType.WILL_CONTACT + "&date="
        + sdf1.format(nextDue) + "\" class=\""
        + (nextActionType.equals(ProjectNextActionType.WILL_CONTACT) ? "box" : "button") + "\">will contact</a></font>");
    out.println("            I have: ");
    out.println("            <font size=\"-1\"><a href=\"HomeServlet?nextActionType=T&date="
        + sdf1.format(nextDue)
        + "\" class=\""
        + ((nextActionType.equals(ProjectNextActionType.COMMITTED_TO) || nextActionType
            .equals(ProjectNextActionType.OVERDUE_TO)) ? "box" : "button") + "\">committed</a>,");
    ...
    out.println("            I am:");
    out.println("            <font size=\"-1\"><a href=\"HomeServlet?nextActionType=W&date=" + sdf1.format(nextDue)
        + "\" class=\"" + (nextActionType.equals("W") ? "box" : "button") + "\">waiting</a>,");
    ...
    calendar.add(Calendar.DAY_OF_MONTH, 1);
    out.println("              <a href=\"javascript: void setNextAction('" + sdf1.format(calendar.getTime())
        + "');\" class=\"" + (sameDay(calendar, nextDue) ? "box" : "button") + "\">" + day.format(calendar.getTime())
        + "</a>");
    ...
    for (int i = 0; i < 6; i++) {
      ...
      if (nextWeek) {
        out.println("              <a href=\"javascript: void setNextAction('" + sdf1.format(calendar.getTime())
            + "');\" class=\"" + (sameDay(calendar, nextDue) ? "box" : "button") + "\">Next-"
  ...
  protected static List<ProjectAction> prepareProjectActionListAndIdentifyOverdue(Session dataSession,
...

    {
      ...
      for (ProjectAction projectAction : projectActionList) {
        ...
        projectAction.setContact((ProjectContact) dataSession.get(ProjectContact.class, projectAction.getContactId()));
  ...
  private static void printDueTable(WebUser webUser, PrintWriter out, SimpleDateFormat sdf1, String nextActionType,
      Date nextDue, List<ProjectAction> projectActionList, Calendar cIndicated, Calendar cToday, Calendar cTomorrow,
      boolean showLink) {
    out.println("<table class=\"boxed\">");
    out.println("  <tr class=\"boxed\">");
    ...
    for (ProjectAction projectAction : projectActionList) {
      if (projectAction.getNextActionType() != null
        ...
        printActionLine(webUser, out, sdf1, nextActionType, nextDue, cIndicated, projectAction, showLink);
      }
    }
    for (ProjectAction projectAction : projectActionList) {
      if (projectAction.getNextActionType() != null
    ...
    for (ProjectAction projectAction : projectActionList) {
      if (projectAction.getNextActionType() != null
          && (projectAction.getNextActionType().equals(ProjectNextActionType.WILL) || projectAction.getNextActionType()
              .equals(ProjectNextActionType.WILL_CONTACT))) {
        printActionLine(webUser, out, sdf1, nextActionType, nextDue, cIndicated, projectAction, showLink);
      }
    }
    for (ProjectAction projectAction : projectActionList) {
  ...
  private static void printActionLine(WebUser webUser, PrintWriter out, SimpleDateFormat sdf1, String nextActionType,
      Date nextDue, Calendar cIndicated, ProjectAction projectAction, boolean showLink) {
    if (!sameDay(cIndicated, projectAction.getNextDue())) {
      return;
    }
    if (!nextActionType.equals("") && !nextActionType.equals(projectAction.getNextActionType())) {
    ...
    if (projectAction.getNextTimeEstimate() == null || projectAction.getNextTimeEstimate() == 0) {
      ...
      out.println("    <td class=\"boxed\">" + projectAction.getNextTimeEstimateForDisplay() + "</a></td>");
    }
  ...
  private static void printActionOverdueLine(WebUser webUser, PrintWriter out, SimpleDateFormat sdf1,
    ...
    if (projectAction.getNextTimeEstimate() != null && projectAction.getNextTimeEstimate() > 0) {
      out.println("    <td class=\"boxed\">" + projectAction.getNextTimeEstimateForDisplay() + "</td>");
    } else {
  ...
  private static String printOutAction(WebUser webUser, PrintWriter out, SimpleDateFormat sdf1, String nextActionType,
      Date nextDue, ProjectAction projectAction, boolean showLink) {
    ...
    if (showLink) {
      out.println("    <td class=\"boxed\"><a href=\"javascript: void toggleLayer('" + changeBoxId
          + "'); \" class=\"button\">" + projectAction.getNextDescriptionForDisplay(webUser.getProjectContact())
          + "</a>");
    } else {
      out.println("    <td class=\"boxed\">" + projectAction.getNextDescriptionForDisplay(webUser.getProjectContact()));
    }
    ...
    if (showLink) {
      ...
      out.println("            I: <font size=\"-1\"><a href=\"javascript: void changeNextActionType('"
          + ProjectNextActionType.WILL + "', '" + changeFormId + "'); \" class=\""
          + (projectAction.getNextActionType().equals(ProjectNextActionType.WILL) ? "box" : "button") + "\"> will</a>,");
      out.println("            <a href=\"javascript: void changeNextActionType('M', '" + changeFormId
          + "'); \" class=\"" + (projectAction.getNextActionType().equals("M") ? "box" : "button") + "\">might</a>, ");
      out.println("            <a href=\"javascript: void changeNextActionType('" + ProjectNextActionType.WILL_CONTACT
          + "', '" + changeFormId + "'); \" class=\""
          + (projectAction.getNextActionType().equals(ProjectNextActionType.WILL_CONTACT) ? "box" : "button")
          + "\">will contact</a></font>");
      out.println("            I have: ");
      out.println("            <font size=\"-1\"><a href=\"javascript: void changeNextActionType('"
          ...
          + changeFormId
          + "'); \" class=\""
          + ((projectAction.getNextActionType().equals(ProjectNextActionType.COMMITTED_TO) || projectAction
              .getNextActionType().equals(ProjectNextActionType.OVERDUE_TO)) ? "box" : "button") + "\">committed</a>,");
      out.println("            <a href=\"javascript: void changeNextActionType('G', '" + changeFormId
          + "'); \" class=\"" + (projectAction.getNextActionType().equals("G") ? "box" : "button")
          + "\">set goal</a></font>");
      out.println("            I am:");
      out.println("            <font size=\"-1\"><a href=\"javascript: void changeNextActionType('W', '" + changeFormId
          + "'); \" class=\"" + (projectAction.getNextActionType().equals("W") ? "box" : "button") + "\">waiting</a>,");
      ...
      out.println("           <br> Due <input type=\"text\" name=\"changeNextDue\" value=\""
          + (projectAction.getNextDue() == null ? "" : sdf1.format(projectAction.getNextDue())) + "\" size=\"10\" onchange=\"this.form.submit()\">");
      out.println("            <font size=\"-1\">");
      Calendar calendar = Calendar.getInstance();
      SimpleDateFormat day = new SimpleDateFormat("EEE");
      out.println("              <a href=\"javascript: void changeNextAction('" + sdf1.format(calendar.getTime())
          + "', '" + changeFormId + "');\" class=\""
          + (sameDay(calendar, projectAction.getNextDue()) ? "box" : "button") + "\">Today</a>");
      calendar.add(Calendar.DAY_OF_MONTH, 1);
      out.println("              <a href=\"javascript: void changeNextAction('" + sdf1.format(calendar.getTime())
          + "', '" + changeFormId + "');\" class=\""
          + (sameDay(calendar, projectAction.getNextDue()) ? "box" : "button") + "\">" + day.format(calendar.getTime())
          + "</a>");
      boolean nextWeek = false;
      for (int i = 0; i < 6; i++) {
        calendar.add(Calendar.DAY_OF_MONTH, 1);
      ...
      out.println("           <br> Time <input type=\"text\" name=\"nextTimeEstimate\" value=\""
          + (projectAction.getNextTimeEstimate() != null && projectAction.getNextTimeEstimate() > 0 ? ("" + projectAction
              .getNextTimeEstimate()) : "") + "\" size=\"3\" onchange=\"this.form.submit()\"> mins");
      out.println("            <font size=\"-1\">");
      out.println("              <a href=\"javascript: void changeNextTimeEstimate('5', '" + changeFormId
          + "');\" class=\"button\"> 5m</a>");
      out.println("              <a href=\"javascript: void changeNextTimeEstimate('10', '" + changeFormId
          + "');\" class=\"button\"> 10m</a>");
      out.println("              <a href=\"javascript: void changeNextTimeEstimate('20', '" + changeFormId
          + "');\" class=\"button\"> 20m</a>,");
      out.println("              <a href=\"javascript: void changeNextTimeEstimate('40', '" + changeFormId
          + "');\" class=\"button\"> 40m</a>");
      out.println("              <a href=\"javascript: void changeNextTimeEstimate('80', '" + changeFormId
          + "');\" class=\"button\"> 80m</a>");
      out.println("              <a href=\"javascript: void changeNextTimeEstimate('120', '" + changeFormId
          + "');\" class=\"button\"> 2h</a>");
      out.println("              <a href=\"javascript: void changeNextTimeEstimate('240', '" + changeFormId
          + "');\" class=\"button\"> 4h</a>");
      out.println("              <a href=\"javascript: void changeNextTimeEstimate('360', '" + changeFormId
          + "');\" class=\"button\"> 6h</a>");
      out.println("            </font>");
