...
public class ProjectServlet extends ClientServlet
{
  ...
  protected void processRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
  {
    ...
    try
    {
      ...
      ProjectContactAssigned projectContactAssignedForThisUser = getProjectContactAssigned(webUser, dataSession, project);
      ...
      if (action != null)
      {
        if (action.equals("Save"))
        {
          ...
          projectAction.setNextActionType(request.getParameter("nextActionType"));
          ...
          if (message == null)
          {
            ...
            if (sendEmailToList.size() > 0)
            {
              ...
              {
                msg.append("<p>");
                msg.append(projectAction.getNextDescriptionForDisplay());
      ...
      if (projectIdList != null && projectIdList.size() > 1)
      {
        ...
        if (prev != 0)
        {
          out.println("    <font size=\"-1\"><a href=\"ProjectServlet?projectId=" + prev + "\" class=\"box\">Prev</a></font>");
        }
        out.println((pos + 1) + " of " + projectIdList.size());
        if (next != 0)
        {
          out.println("    <font size=\"-1\"><a href=\"ProjectServlet?projectId=" + next + "\" class=\"box\">Next</a></font>");
        }
        out.println("</div>");
      }
      ...
      for (Project p : projectSelectedList)
      {
        ...
        out.println("    <td class=\"boxed\"><a href=\"ProjectServlet?projectId=" + p.getProjectId() + "\" class=\"button\">" + p.getProjectName()
            + "</a></td>");
        out.println("  </tr>");
      }
      ...
      out.println("<table class=\"boxed-fill\">");
      out.println("  <tr>");
      out.println("    <th class=\"title\" colspan=\"2\"><span class=\"right\"><font size=\"-1\"><a href=\"ProjectEditServlet?projectId=" + projectId
          + "\" class=\"box\">Edit</a></font></span>Project Information</th>");
      out.println("  </tr>");
      out.println("  <tr class=\"boxed\">");
      ...
      if (projectContactAssignedForThisUser != null)
      {
        if (projectContactAssignedForThisUser.getUpdateDue() > 0)
        {
          ...
          out.println("    <td class=\"boxed\">" + ProjectReviewServlet.makeLabel(projectContactAssignedForThisUser.getUpdateDue()) + "</td>");
          out.println("  </tr>");
        }
      }
      out.println("</table>");
      ...
      for (ProjectContactAssigned projectContactAssigned : projectContactAssignedList)
      {
        ...
        if (projectContact.getEmail() != null && !projectContact.getEmail().equals(""))
        {
          ...
          out.println(" <font size=\"-1\"><a href=\"mailto:" + projectContact.getEmail() + "?subject=" + encode(projectName + " Project")
              + "\" class=\"box\">Email</a></font>");
        }
        out.println(" <font size=\"-1\"><a href=\"ProjectServlet?action=RemoveContact&projectId=" + projectId + "&contactId="
            + projectContact.getContactId() + "\" class=\"box\">Remove</a></font>");
        out.println("</td>");
        out.println("  </tr>");
      }
      out.println("</table>");
      ...
      if (projectActionList.size() > 0)
      {
        ...
        for (ProjectAction projectAction1 : projectActionList)
        {
          ...
          out.println("    <td class=\"inside\"><input type=\"checkbox\" name=\"completed\" value=\"" + projectAction1.getActionId() + "\"></td>");
          out.println("  </tr>");
        }
        out.println("</table>");
      } else
      ...
      out.println("          <td class=\"inside\"><input type=\"text\" name=\"actionDate\" size=\"20\" value=\"" + sdf1.format(new Date())
          + "\" onkeydown=\"resetRefresh()\"></td>");
      out.println("        </tr>");
      out.println("        <tr>");
      ...
      out.println("              <a href=\"javascript: void setNextAction('" + sdf1.format(calendar.getTime()) + "');\" class=\"button\">Today</a>");
      calendar.add(Calendar.DAY_OF_MONTH, 1);
      out.println("              <a href=\"javascript: void setNextAction('" + sdf1.format(calendar.getTime()) + "');\" class=\"button\">"
      ...
      for (int i = 0; i < 6; i++)
      {
        ...
        if (nextWeek)
        {
          out.println("              <a href=\"javascript: void setNextAction('" + sdf1.format(calendar.getTime()) + "');\" class=\"button\">Next-"
              + day.format(calendar.getTime()) + "</a>");
        } else
        {
          out.println("              <a href=\"javascript: void setNextAction('" + sdf1.format(calendar.getTime()) + "');\" class=\"button\">"
      ...
      for (ProjectContact projectContact1 : projectContactList)
      {
        if (projectContact1.getContactId() != webUser.getProjectContact().getContactId())
        {
          ...
          out.println("                  <option value=\"" + projectContact1.getContactId() + "\"" + (selected ? " selected" : "") + ">"
              + projectContact1.getName() + "</option>");
        }
      }
      out.println("            </select>");
      out.println("          </td>");
      ...
      for (ProjectContact projectContact1 : projectContactList)
      {
        out.println("          <span class=\"together\"><input type=\"checkbox\" name=\"sendEmailTo" + projectContact1.getContactId()
            + "\" value=\"Y\"/>");
        out.println("<font size=\"-1\"><a href=\"javascript: void clickForEmail('" + projectContact1.getContactId() + "');\" class=\"button\">"
            + projectContact1.getName() + "</a></font></span>");
      }
  ...
  protected static ProjectContactAssigned getProjectContactAssigned(WebUser webUser, Session dataSession, Project project)
  {
    Query query;
