...
public class ReportsServlet extends ClientServlet {
  ...
  protected void processRequest(HttpServletRequest request, HttpServletResponse response)
    ...
    try {
      ...
      for (ReportProfile reportProfile : reportProfileList) {
        ...
        if (reportSchedule != null) {
          out.println("    <td class=\"boxed\">"
              + (reportSchedule.getPeriod() != null ? reportSchedule.getPeriodText() : "")
              + "</td>");
          out.println("    <td class=\"boxed\">"
      ...
      for (ReportProfile reportProfile : extendReportProfileList) {
        out.println("  <li><a href=\"ReportEditServlet?extendsProfileId="
            + reportProfile.getProfileId() + "\">" + reportProfile.getProfileLabel() + "</a></li>");
      }
      out.println("</ul>");
  ...
  protected static void loadReportProfileObject(Session dataSession, ReportProfile reportProfile) {
    ...
    reportProfile.setReportCSS(TrackerKeysManager.getKeyContent(TrackerKeysManager.KEY_REPORT_CSS,
        TrackerKeysManager.KEY_TYPE_REPORT, String.valueOf(reportProfile.getProfileId()),
        dataSession));
  }
