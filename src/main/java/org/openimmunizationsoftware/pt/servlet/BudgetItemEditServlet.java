...
public class BudgetItemEditServlet extends ClientServlet {
  ...
  protected void processRequest(HttpServletRequest request, HttpServletResponse response)
    ...
    try {
      ...
      if (request.getParameter("transId") != null && !request.getParameter("transId").equals("0")
          && !request.getParameter("transId").equals("")) {
        budgetTrans = (BudgetTrans) dataSession.get(BudgetTrans.class,
      ...
      if (budgetTrans == null) {
        ...
        if (request.getParameter("monthDate") != null) {
          try {
            BudgetMonth budgetMonth = BudgetServlet.getBudgetMonth(dataSession, budgetAccount,
                sdf.parse(request.getParameter("monthDate")));
      ...
      if (budgetItem == null && request.getParameter("itemId") != null
          && !request.getParameter("itemId").equals("0")
          && !request.getParameter("itemId").equals("")) {
        budgetItem = (BudgetItem) dataSession.get(BudgetItem.class,
        ...
        if (action.equals("Save")) {
          ...
          if (budgetItem.getItemId() == 0) {
            Query query = dataSession
                .createQuery("from BudgetItem where itemLabel = ? and budgetAccount = ?");
            ...
            List<BudgetItem> budgetItemAreadyExistsList = query.list();
            if (budgetItemAreadyExistsList.size() > 0
                && budgetItemAreadyExistsList.get(0).getItemId() != budgetItem.getItemId()) {
              message = "Unable to save item label is not unique";
            }
          }
          if (message != null) {
            ...
            if (relatedItemIdString != null && !relatedItemIdString.equals("0")
                && !relatedItemIdString.equals("") && !relatedItemIdString.startsWith("-")) {
              budgetItem.setRelatedBudgetItem((BudgetItem) dataSession.get(BudgetItem.class,
              ...
              try {
                ...
                if (budgetTrans.getTransStatus() != BudgetTrans.TRANS_STATUS_PAID) {
                  if (budgetTrans.getBudgetMonth() == null) {
                    budgetTrans.setBudgetMonth((BudgetMonth) dataSession.get(BudgetMonth.class,
                        Integer.parseInt(request.getParameter("monthId"))));
                  }
                  try {
                  ...
                  if (message != null) {
                    ...
                    if (relatedItemIdString != null && relatedItemIdString.startsWith("-")) {
                      ...
                      BudgetAccount relatedBudgetAccount = (BudgetAccount) dataSession
                          .get(BudgetAccount.class, -Integer.parseInt(relatedItemIdString));
                    ...
                    if (budgetItem.getRelatedBudgetItem() != null) {
                      ...
                      if (relatedBudgetTrans == null) {
                        Query query = dataSession.createQuery(
                            "from BudgetTrans where budgetItem = ? and budgetMonth.monthDate = ?");
                      ...
                      BudgetMonth relatedBudgetMonth = BudgetServlet.getOrCreateBudgetMonth(
                          dataSession, budgetItem.getRelatedBudgetItem().getBudgetAccount(),
                          budgetTrans.getBudgetMonth().getMonthDate());
                      if (relatedBudgetTrans == null) {
                        ...
                        if (!relatedBudgetTrans.getTransStatus()
                            .equals(BudgetTrans.TRANS_STATUS_PAID)) {
                          relatedBudgetTrans.setBudgetMonth(relatedBudgetMonth);
                          relatedBudgetTrans.setRelatedBudgetTrans(budgetTrans);
                          relatedBudgetTrans.setTransAmount(-budgetTrans.getTransAmount());
                          relatedBudgetTrans.setTransDate(budgetTrans.getTransDate());
                    ...
                    response.sendRedirect("BudgetServlet?accountId=" + budgetAccount.getAccountId()
                        + "&monthDate=" + sdf.format(budgetTrans.getBudgetMonth().getMonthDate()));
                    return;
                  }
                }
              } finally {
      ...
      out.println(
          "<input type=\"hidden\" name=\"transId\" value=\"" + budgetTrans.getTransId() + "\">");
      ...
      Query query =
          dataSession.createQuery("from BudgetItem where budgetAccount = ? order by itemLabel");
      ...
      for (BudgetItem selectBudgetItem : selectBudgetItemList) {
        if (selectBudgetItem.getItemId() == budgetItem.getItemId()) {
          out.println("      <option value=\"" + selectBudgetItem.getItemId() + "\" selected>"
              + selectBudgetItem.getItemLabel() + "</option>");
        } else {
          out.println("      <option value=\"" + selectBudgetItem.getItemId() + "\">"
              + selectBudgetItem.getItemLabel() + "</option>");
        }
      }
      out.println("      </select>");
      ...
      out.println("    <td class=\"boxed\"><input type=\"text\" name=\"itemLabel\" value=\""
          + budgetItem.getItemLabel() + "\" size=\"30\"></td>");
      out.println("  </tr>");
      out.println("  <tr class=\"boxed\">");
      ...
      for (String[] selection : BudgetItem.ITEM_STATUS) {
        if (selection[0].equals(budgetItem.getItemStatus())) {
          out.println("      <option value=\"" + selection[0] + "\" selected>" + selection[1]
              + "</option>");
        } else {
      ...
      for (BudgetItem relatedBudgetItem : relatedBudgetItemList) {
        if (budgetItem.getRelatedBudgetItem() != null
            && relatedBudgetItem.getItemId() == budgetItem.getRelatedBudgetItem().getItemId()) {
          out.println("      <option value=\"" + relatedBudgetItem.getItemId() + "\" selected>"
              + relatedBudgetItem.getBudgetAccount().getAccountLabel() + ": "
              + relatedBudgetItem.getItemLabel() + "</option>");
        } else {
          out.println("      <option value=\"" + relatedBudgetItem.getItemId() + "\">"
              + relatedBudgetItem.getBudgetAccount().getAccountLabel() + ": "
              + relatedBudgetItem.getItemLabel() + "</option>");
        }
      }
      ...
      for (BudgetAccount lastBudgetAccount : relatedBudgetAccountList) {
        if (lastBudgetAccount.getAccountId() != budgetAccount.getAccountId()) {
          out.println("      <option value=\"-" + lastBudgetAccount.getAccountId() + "\">"
              + lastBudgetAccount.getAccountLabel() + ": (new) </option>");
        }
      }
      out.println("      </select>");
      out.println("    </td>");
      ...
      if (budgetItem.getLastAmount() > 0) {
        ...
        out.println(
            "    <td class=\"boxed\">" + MoneyUtil.format(budgetItem.getLastAmount()) + "</td>");
        out.println("  </tr>");
        out.println("  <tr class=\"boxed\">");
      ...
      if (budgetTrans.getTransStatus() != BudgetTrans.TRANS_STATUS_PAID) {
        ...
        for (BudgetMonth budgetMonth : budgetMonthList) {
          if (budgetTrans.getBudgetMonth() != null
            ...
            out.println("      <option value=\"" + budgetMonth.getMonthId() + "\" selected>"
                + monthSdf.format(budgetMonth.getMonthDate()) + "</option>");
          } else {
            out.println("      <option value=\"" + budgetMonth.getMonthId() + "\">"
                + monthSdf.format(budgetMonth.getMonthDate()) + "</option>");
          }
        }
        out.println("      </select>");
        ...
        out.println("    <td class=\"boxed\"><input type=\"text\" name=\"transDate\" value=\""
            + sdf.format(budgetTrans.getTransDate()) + "\" size=\"10\"></td>");
        out.println("  </tr>");

        out.println("  <tr class=\"boxed\">");
        out.println("    <th class=\"boxed\">Amount</th>");
        out.println("    <td class=\"boxed\"><input type=\"text\" name=\"transAmount\" value=\""
            + MoneyUtil.format(budgetTrans.getTransAmount()) + "\" size=\"10\"></td>");
        out.println("  </tr>");
        out.println("  <tr class=\"boxed\">");
        ...
        out.println("    <th class=\"boxed\">Amount</th>");
        out.println(
            "    <td class=\"boxed\">" + MoneyUtil.format(budgetTrans.getTransAmount()) + "</td>");
        out.println("  </tr>");
        out.println("  <tr class=\"boxed\">");
        out.println("    <th class=\"boxed\">Trans Status</th>");
      ...
      if (budgetItem.getItemId() > 0) {
        ...
        out.println("  </tr>");
        query = dataSession.createQuery(
            "from BudgetTrans where budgetItem = ? order by budgetMonth.monthDate desc");
        ...
        for (BudgetTrans otherBudgetTrans : budgetTransList) {
          ...
          out.println("    <td class=\"boxed\"><a href=\"BudgetItemEditServlet?accountId="
              + budgetAccount.getAccountId() + "&monthDate="
              + sdf.format(MonthUtil.getMonthDate(otherBudgetTrans.getTransDate())) + "&transId="
              + otherBudgetTrans.getTransId() + "\" class=\"button\">"
              + sdf.format(otherBudgetTrans.getTransDate()) + "</a></td>");
          out.println(
              "    <td class=\"boxed\">" + otherBudgetTrans.getTransStatusLabel() + "</td>");
          out.println("    <td class=\"boxed\">"
              + MoneyUtil.format(otherBudgetTrans.getTransAmount()) + "</td>");
          out.println("  </tr>");
