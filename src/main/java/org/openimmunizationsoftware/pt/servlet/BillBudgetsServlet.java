...
public class BillBudgetsServlet extends ClientServlet
{
        ...
        if (action.equals("Update Time")) {
          ...
          while (enumeration.hasMoreElements()) {
            ...
            if (paramName.startsWith("wd")) {
              ...
              Query query = dataSession.createQuery("from BillExpected where id.username = ? and id.billDate = ?");
  ...
  public static void runReport(WebUser webUser, PrintWriter out, Session dataSession, boolean showLinks) {
    ...
    for (BillBudget billBudget : billBudgetList) {
      ...
      out.println("  <tr class=\"boxed\">");
      if (showLinks) {
        out.println("    <td class=\"boxed\"><a href=\"BillCodeServlet?billCode=" + billCode.getBillCode()
            + "\" class=\"button\">" + n(billCode.getBillLabel()) + "</a></td>");
        out.println("    <td class=\"boxed\"><a href=\"BillBudgetServlet?billBudgetId=" + billBudget.getBillBudgetId()
            + "\" class=\"button\">" + billBudget.getBillBudgetCode() + "</a></td>");
      } else {
    ...
    out.println("<table class=\"boxed\">");
    out.println("  <tr>");
    out.println("    <th class=\"title\" colspan=\"" + (monthDateList.size() + 2) + "\">Remaining Hours</th>");
    out.println("  </tr>");
    out.println("  <tr class=\"boxed\">");
    out.println("    <th class=\"boxed\">Bill Label</th>");
    out.println("    <th class=\"boxed\">" + sdfMonth.format(monthDateList.get(0)) + " Actual</th>");
    for (int i = 1; i < monthDateList.size(); i++) {
    ...
    for (BillBudget billBudget : billBudgetList) {
      ...
      for (int i = 1; i < monthDateList.size(); i++) {
        ...
        if (billMonthList.size() == 0) {
          ...
          if (i == 1) {
            out.println("    <td class=\"boxed\">" + TimeTracker.formatTime(billMonth.getBillMinsActual()) + "</td>");
          }
          out.println("    <td class=\"boxed\">" + TimeTracker.formatTime(billMinsPending) + "</td>");
          totalMins[i] += billMinsPending;
          totalBillings[i] += (int) (billMinsPending * billMonth.getBillBudget().getBillCode().getBillRate() / 100.0 + 0.5);
        }
        startDate = endDate;
      }
    ...
    for (int i = 1; i < monthDateList.size(); i++) {
      if (workingDayCountsPerMonth[i] != 0) {
        out.println("    <td class=\"boxed\">" + TimeTracker.formatTime(totalMins[i] / workingDayCountsPerMonth[i])
            + "</td>");
      } else {
        out.println("    <td class=\"boxed\">&nbsp;</td>");
      }
    ...
    for (int i = 1; i < monthDateList.size(); i++) {
      if (workingDayCountsPerMonth[i] != 0) {
        out.println("    <td class=\"boxed\">$" + TrackServlet.formatMoney(totalBillings[i]) + "</td>");
      } else {
        out.println("    <td class=\"boxed\">&nbsp;</td>");
      }
    }
    ...
    for (WorkingDay workingDay : workingDayList[1]) {
      ...
      if (timeTracker.getTotalMinsBillable() > 0) {
        ...
        out.println("    <td class=\"boxed\">$" + TrackServlet.formatMoney(totalBillableMoney) + "</td>");
      } else {
        out.println("    <td class=\"boxed\">&nbsp;</td>");
        out.println("    <td class=\"boxed\">&nbsp;</td>");
        out.println("    <td class=\"boxed\">&nbsp;</td>");
    ...
    out.println("    <td class=\"boxed\">$" + TrackServlet.formatMoney(totalBillableMoneyRunning) + "</td>");
    out.println("  </tr>");
    ...
    for (WorkingDay workingDay : workingDayList[1]) {
      if (!workingDay.date.before(today)) {
        ...
        if (showLinks) {
          ...
          out.println("    <td class=\"boxed\">" + TimeTracker.formatTime(billExpected.getBillMins()) + "</td>");
        }
        out.println("  </tr>");
    ...
    if (showLinks) {
      out.println("  <tr>");
      out.println("    <td class=\"boxed\"></td>");
      out.println("    <td class=\"boxed\"><input type=\"submit\" name=\"action\" value=\"Update Time\"/></td>");
      out.println("  </tr>");
    }
    out.println("</table> ");
    if (showLinks) {
    ...
    out.println("    <td class=\"boxed\">" + TimeTracker.formatTime(timeLeftToWork) + "</td>");
    out.println("    <td class=\"boxed\">$" + TrackServlet.formatMoney(additionalBillable) + "</td>");
    out.println("  </tr>");
    out.println("  <tr class=\"boxed\">");
    out.println("    <th class=\"boxed\">Low Estimate</th>");
    out.println("    <td class=\"boxed\">" + TimeTracker.formatTime(totalTimeRunning + timeLeftToWork) + "</td>");
    out.println("    <td class=\"boxed\">$" + TrackServlet.formatMoney(totalBillableMoneyRunning + additionalBillable)
        + "</td>");
    out.println("  </tr>");
    out.println("  <tr class=\"boxed\">");
    out.println("    <th class=\"boxed\">High Estimate</th>");
    out.println("    <td class=\"boxed\">" + TimeTracker.formatTime(totalTimeRunning + totalMins[1]) + "</td>");
    out.println("    <td class=\"boxed\">$" + TrackServlet.formatMoney(totalBillableMoneyRunning + totalBillings[1])
        + "</td>");
    out.println("  </tr>");
    out.println("</table>");
  }
