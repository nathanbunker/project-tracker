...
public class FocusedActionPageRenderer {
        ...
        public void render(AppReq appReq, ProjectActionNext action, List<String> notes,
                ...
                out.println("        <div class=\"fa-card fa-next-action-card\">");
                ...
                out.println("        <div id=\"fa-today-time\" class=\"fa-time-value\">" + formatMinutes(spentMinutes)
                                + "</div>");
                ...
                out.println("    var todayPct = faClampPct((spent / todayBase) * 100);");
        ...
        private void printStyles(PrintWriter out) {
                out.println("<style>");
                out.println(
                                "  .fa-page { height: 100vh; overflow: hidden; background: linear-gradient(160deg, #f6f8f5 0%, #edf2ec 48%, #f8fbf7 100%); box-sizing: border-box; padding: 16px; position: relative; }");
                out.println(
                ...
                out.println(
                                "  .fa-shell { display: grid; grid-template-columns: minmax(380px, 40%) minmax(540px, 60%); gap: 16px; height: calc(100vh - 32px); }");
                out.println("  .fa-left { min-height: 0; }");
                out.println("  .fa-left-scroll { height: 100%; overflow-y: auto; padding-right: 8px; }");
                out.println(
                ...
                out.println("  .fa-controls { position: absolute; left: 15%; top: 84%; transform: translate(-50%, -50%); z-index: 10; text-align: left; }");
                out.println(
                ...
                out.println("  .fa-next-action-card { border-style: dashed; }");
                ...
                out.println("    .fa-controls { left: 16%; top: 82%; }");
                out.println("    .fa-time-widget { left: 84%; top: 82%; width: min(52vw, 236px); grid-template-columns: auto auto 1fr; }");
