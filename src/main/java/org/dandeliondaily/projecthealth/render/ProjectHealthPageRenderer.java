...
public class ProjectHealthPageRenderer {
    ...
    private void printReportColumn(PrintWriter out, ProjectHealthPageModel model) {
        ...
        out.println("    <div class=\"ph-key\">Category</div><div class=\"ph-val\">" + escapeHtml(report.getCategory())
                + "</div>");
        out.println("    <div class=\"ph-key\">Phase</div><div class=\"ph-val\">" + escapeHtml(report.getPhase())
                + "</div>");
    ...
    private void printReportActionList(PrintWriter out, List<ProjectReportModel.ReportActionLine> lines, String empty) {
    ...
    private void printIssuesAndActions(PrintWriter out, ProjectHealthPageModel model) {
        ...
        if (!model.isSelectedProjectAvailable()) {
            ...
            for (ProjectHealthIssueModel issue : model.getIssues()) {
                ...
                out.println("    <li class=\"" + severityClass + "\"><strong>" + escapeHtml(issue.getTitle())
                        + "</strong><br/>"
    ...
    private void printProjectEditModal(PrintWriter out, AppReq appReq, int selectedProjectId) {
        ...
        for (ProjectPhase phase : phaseList) {
            String selected = phase.getPhaseCode() != null && phase.getPhaseCode().equals(project.getPhaseCode())
                    ? " selected"
                    : "";
    ...
    private void printScripts(PrintWriter out, ProjectHealthPageModel model) {
        ...
        out.println(
                "    var map = [['phProjectName','projectName'],['phProjectCategory','categoryCode'],['phProjectPriority','priorityLevel'],['phProjectIcon','projectIcon'],['phProjectDescription','description'],['phProjectPhase','phaseCode'],['phProjectBillCode','billCode'],['phProjectUpdateEvery','updateEvery']];");
        out.println(
